...
**如果 issue 中没有明确列出关联的测试用例或测试文件路径：**

1. 用 `Skill` 工具调 `codex`，告知 Codex：
   - 当前 issue 的编号和标题
   - 需要实现的功能或修复的内容
   - 目前缺少对应的测试用例
2. 让 Codex 先了解情况，讨论测试策略，落好测试用例
3. 与 Codex 达成共识后，再将测试文件路径记录到 issue 文档中
4. **不跳过这一步直接写实现代码。** 没有测试覆盖的实现不可验收

如果 issue 中已有关联的测试文件路径，跳过此步，进入下一步。
...
### 10. 如果用户明确表示自动运行，或者把所有的issues都清理掉。那么就从头开始执行当前流程，直到所有issues都修复。
